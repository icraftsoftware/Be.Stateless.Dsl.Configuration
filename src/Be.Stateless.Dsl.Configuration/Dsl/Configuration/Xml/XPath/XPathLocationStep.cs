...
namespace Be.Stateless.Dsl.Configuration.Xml.XPath
{
	public class XPathLocationStep
	{
		...
		private static string GetElementName(Match result)
		{
			...
			for (var index = 0; index < result.Groups[ATTRIBUTE_NAME_GROUP].Captures.Count; index++)
			{
				if (result.Groups[ATTRIBUTE_NAME_GROUP].Captures[index].Value == XpathFunctionNames.LOCAL_NAME) return result.Groups[ATTRIBUTE_VALUE_GROUP].Captures[index].Value;
			}
